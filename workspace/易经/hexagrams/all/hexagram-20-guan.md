...
order: 20
description: "Hexagram 20 觀 as `000011`, fold-required, categories 止静与内省 / 明照与分辨."
...
在当前的 Omega 文化映射计划里，第 20 卦 觀 首先不是被当作抽象象义，而是被当作二元词 `000011` 来读取。该卦不在 `X_6` 内，因此其第一层数学位置是 raw 6-bit word，而不是 stable word。 它虽不是极端全阳，但已出现连续阳段，因此其形式位置是 fold 之前的临界或过载态。 它目前横跨的主题类别是 止静与内省、明照与分辨，因此其 strongest reading corridor 集中在 golden-mean-shift、zeckendorf-representation、rate-distortion-information-theory、spectral-theory、modular-tower-inverse-limit 这些方向上。
...
- 当前主方向：golden-mean-shift, zeckendorf-representation, rate-distortion-information-theory, spectral-theory, modular-tower-inverse-limit
...
- 本仓库原文文件：`texts/yijing/hexagram_20_guan.txt`
