...
order: 10
description: "Hexagram 10 履 as `110111`, fold-required, categories 刚健与突破."
...
在当前的 Omega 文化映射计划里，第 10 卦 履 首先不是被当作抽象象义，而是被当作二元词 `110111` 来读取。该卦不在 `X_6` 内，因此其第一层数学位置是 raw 6-bit word，而不是 stable word。 它包含长阳串，因此第一层读法不是 stable word，而是需要经 fold 才能进入稳定域的 raw word。 它目前横跨的主题类别是 刚健与突破，因此其 strongest reading corridor 集中在 fold-operator、fiber-structure、golden-mean-shift 这些方向上。
...
- 本仓库原文文件：`texts/yijing/hexagram_10_lu.txt`
