...
order: 17
description: "Hexagram 17 隨 as `100110`, fold-required, categories 交感与关系."
...
在当前的 Omega 文化映射计划里，第 17 卦 隨 首先不是被当作抽象象义，而是被当作二元词 `100110` 来读取。该卦不在 `X_6` 内，因此其第一层数学位置是 raw 6-bit word，而不是 stable word。 它虽不是极端全阳，但已出现连续阳段，因此其形式位置是 fold 之前的临界或过载态。 它目前横跨的主题类别是 交感与关系，因此其 strongest reading corridor 集中在 ring-arithmetic、spectral-theory、fold-operator 这些方向上。
...
- 本仓库原文文件：`texts/yijing/hexagram_17_sui.txt`
