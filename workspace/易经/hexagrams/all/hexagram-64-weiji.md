...
order: 64
description: "Hexagram 64 未濟 as `010101`, GMS-valid, categories 动态变易与循环 / 节制与平衡."
...
在当前的 Omega 文化映射计划里，第 64 卦 未濟 首先不是被当作抽象象义，而是被当作二元词 `010101` 来读取。该卦直接位于 `X_6` 内，因此不需要先经过 fold 才能进入稳定域。 它以三阳达到了 `No11` 允许的最大阳密度，因此是交替或近交替结构中的高密度稳定态。 它目前横跨的主题类别是 动态变易与循环、节制与平衡，因此其 strongest reading corridor 集中在 dynamical-systems、golden-mean-shift、fold-operator、zeckendorf-representation、ring-arithmetic 这些方向上。
...
- 本仓库原文文件：`texts/yijing/hexagram_64_weiji.txt`
