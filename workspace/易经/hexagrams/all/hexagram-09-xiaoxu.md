...
order: 9
description: "Hexagram 9 小畜 as `111011`, fold-required, categories 刚健与突破 / 节制与平衡."
...
在当前的 Omega 文化映射计划里，第 9 卦 小畜 首先不是被当作抽象象义，而是被当作二元词 `111011` 来读取。该卦不在 `X_6` 内，因此其第一层数学位置是 raw 6-bit word，而不是 stable word。 它包含长阳串，因此第一层读法不是 stable word，而是需要经 fold 才能进入稳定域的 raw word。 它目前横跨的主题类别是 刚健与突破、节制与平衡，因此其 strongest reading corridor 集中在 fold-operator、fiber-structure、golden-mean-shift、zeckendorf-representation、ring-arithmetic 这些方向上。
...
- 当前主方向：fold-operator, fiber-structure, golden-mean-shift, zeckendorf-representation, ring-arithmetic
...
- 本仓库原文文件：`texts/yijing/hexagram_09_xiaoxu.txt`
