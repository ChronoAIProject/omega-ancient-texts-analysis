...
order: 8
description: "Hexagram 8 比 as `000010`, GMS-valid, categories 柔顺与养育 / 聚散与流通."
...
在当前的 Omega 文化映射计划里，第 8 卦 比 首先不是被当作抽象象义，而是被当作二元词 `000010` 来读取。该卦直接位于 `X_6` 内，因此不需要先经过 fold 才能进入稳定域。 它只保留一个孤立阳位，因此属于最小非零稳定激活，可视作稀疏启动态。 它目前横跨的主题类别是 柔顺与养育、聚散与流通，因此其 strongest reading corridor 集中在 golden-mean-shift、fibonacci-growth、zeckendorf-representation、rate-distortion-information-theory、fold-operator、modular-tower-inverse-limit 这些方向上。
...
- `Word 6 = {0,1}^6`
- `X_6` stable subspace
- 当前主方向：golden-mean-shift, fibonacci-growth, zeckendorf-representation, rate-distortion-information-theory, fold-operator, modular-tower-inverse-limit
...
- 本仓库原文文件：`texts/yijing/hexagram_08_bi.txt`
